...
$$
\vec J (t+1) = \vec J + \frac{1}{\sqrt{N}} \sigma^{\mu}_T \vec \xi^{\mu} (1+\vec r)
$$
...
plt.plot([1, 10, 20, 50, 100, 150, 200], epsilons_P)
plt.xlabel('$P$')
...
As the size $N$ of input vectors increases, the error decay is reached for higher values of the training set size $P$. In particular for double the size of the input vector, the decay time nearly doubles (it is actually slightly more than the double).

This is what we expect from the fact that (see, for example, Engel & Van den Broek p. 12):

$$
P = \alpha N
$$

where $\alpha$ is of order 1. It is a bit dangerous to make assumptions from two data points (with no regards whatsoever to the propagation of errors, either), but if the latter statement holds we can clearly see that $P_{typical}$ grows linearly with $N$. In our case $\alpha$ would have a value of
...
print('Alphas for N=20 and N=40 are', alpha20, alpha40)
```
